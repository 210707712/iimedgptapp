<template>
  <div class="home">
    ...
    <div class="home-right">
      <div class="right-version">
        <div class="llm-chat-demo">
          <span class="chat-demo">基于IIMedGPT的大语言模型肿瘤问答平台</span>
      ...
      <div class="right-input" @keyup.enter="handleSearch">
        <!-- 输入框 -->
        <el-input v-model="queryKeyword" placeholder="给IIMedGPT发送消息" class="input"></el-input>
        <!-- 查询按钮 -->
    ...
    <el-button type="text" class="close-button" @click="toggleSettings">关闭</el-button>
      ...
      <div class="sec-notice">肿瘤问答平台，生成内容仅供学习，具体病情请询问医师。</div>
    </div>
  </div>
</template>
...
import zlzhishi from './zlzhishi'; // 确保路径正确
import axios from 'axios';
export default {
  name: 'HomeView',
  components: {body1,zlzhishi},
  computed: {
  ...
  data() {
    return {
      ...
      { value: '1', label: 'IIMedGPT-v1.0' },
        { value: '2', label: 'IIMedGPT-v2.0' }
      ],
  ...
  methods: {
    fetchData() {
      axios.get('http://202.127.200.31:30012/llm/zhishi2?query='+this.queryKeyword)
  ...
  // documents 已不为空，继续执行后续操作...
    ...
    console.error('Error waiting for documents:', error);
        ...
        if (this.modelselectedOption == '1') {
          this.eventSource = new EventSource('http://202.127.200.31:30012/llm/requestIIMedGPT?query=' +"请参考以下知识: "+this.documents[0].text+"。 回答问题: "+ keyword+'&prompt=' + this.settingInput);
          } else if (this.modelselectedOption == '2') {
          this.eventSource = new EventSource('http://202.127.200.31:30012/llm/requestIIMedGPT2?query=' +"请参考以下知识: "+this.documents[0].text+"。 回答问题: "+ keyword+'&prompt=' + this.settingInput);
          }else {
          // this.eventSource = new EventSource('http://202.127.200.31:30012/llm/requestIIMedGPT?query=' +"请参考以下知识: "+this.documents[0].text+"。 回答问题: "+ keyword+'&prompt=' + this.settingInput);
          this.eventSource = new EventSource('http://202.127.200.31:30012/llm/requestIIMedGPT2?query=' +"请参考以下知识: "+this.documents[0].text+"。 回答问题: "+ keyword+'&prompt=' + this.settingInput);
          // this.eventSource = new EventSource('http://127.0.0.1:5000/llm/requestIIMedGPT2?query=' +"请参考以下知识: "+this.documents[0].text+"。 回答问题: "+ keyword+'&prompt=' + this.settingInput);
