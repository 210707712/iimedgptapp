...
class ApiMemoryChat(BaseMemoryChat):
    ...
    def chat_with_memory(self,
        ...
        # 调用memory_service的retrieve_memory操作来获取记忆
        # retrieve_memory是在BaseMemoryService中定义的抽象方法,具体实现在MemoryScopeService中
        ...
        time_format = self.prompt_handler.system_prompt.format(date_time=f"{date_time} {weekday}")
        if system_prompt:
            system_prompt_list.append(str(system_prompt)+","+time_format)
        else:
            system_prompt_list.append(time_format)
