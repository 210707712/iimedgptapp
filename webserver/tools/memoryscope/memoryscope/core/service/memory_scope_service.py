...
class MemoryScopeService(BaseMemoryService):
    def __init__(self,
        ...
        Args:
            ...
            history_msg_count (int): 内存中保存的对话历史数量,用于控制数量并减少内存使用。
            contextual_msg_max_count (int): 一次对话中的最大上下文长度。如果超过这个长度,
                    将不会被包含在上下文中以防止token溢出。
            contextual_msg_min_count (int): 一次对话中的最小上下文长度。如果短于这个长度,
                    将不会生成对话摘要和长期记忆。
            kwargs (dict): 用于自定义服务行为的额外参数。
        """
