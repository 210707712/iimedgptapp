...
# from memoryscope.core.operation.base_workflow import BaseNodePostprocessor
from pydantic import Field
from typing import Optional
import requests


class BGERerank(BaseModel):
    ...
    def rerank(self, nodes: List[Node]):
        headers = {"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}
        payload = {"nodes": [node.text for node in nodes], "top_n": self.top_n}  # 示例 payload，根据你的需要调整
        response = requests.request("POST", self.api_base, json=payload, headers=headers)
